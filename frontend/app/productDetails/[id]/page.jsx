...
import { createReview, getproductDetails, removeError, removeSuccess } from 'app/features/products/productSlice';
import { addtoCart, removeMessage } from 'app/features/cart/cartSlice';
...
const ProductDetails = () => {
  ...
  const addToCartHandler = () => dispatch(addtoCart({ id, quantity }));
          ...
          <>
              ...
              <div className="flex-1 space-y-4">
                ...
                {product.stock > 0 && (
                  <>
                    ...
                    <button onClick={addToCartHandler} disabled={cartLoading} className="w-full mt-6 bg-gray-700 text-white py-3 rounded-lg hover:bg-gray-900 shadow-md hover:shadow-lg transform hover:scale-105 transition font-medium">{cartLoading ? 'Adding...' : 'Add to Cart'}</button>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="mb-12 bg-white shadow-md rounded-xl p-6 hover:shadow-lg transition duration-300">
              ...
              <form onSubmit={handleSubmitReview}>
                ...
                <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Write your review here..." className="w-full mt-4 p-3 border rounded-md shadow-sm hover:shadow-md transition focus:ring-2 focus:ring-gray-300 focus:outline-none" rows={4} required />
                <button type="submit" disabled={reviewLoading} className="mt-4 bg-gray-700 text-white py-2 px-6 rounded-md hover:bg-gray-900 shadow-md hover:shadow-lg transform hover:scale-105 transition font-medium">{reviewLoading ? 'Submitting...' : 'Submit Review'}</button>
              </form>
            </div>
